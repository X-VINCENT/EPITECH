...
{
    if (nb_size == 1)
        modified_str = my_strcat(modified_str, "00");
    if (nb_size == 2)
        modified_str = my_strcat(modified_str, "0");
    return modified_str;
}
...
char *mod_str(char const *str)
{
    ...
    while (str[i] != '\0') {
        if (str[i] >= 32 && str[i] <= 128)
            ...
            modified_str = mod_str_bis(modified_str, my_strlen(tmp_str));
            modified_str = my_strcat(modified_str, tmp_str);
            j += 3;
        }
        i += 1;
        j += 1;
